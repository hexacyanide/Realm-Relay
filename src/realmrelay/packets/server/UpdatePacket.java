...
public class UpdatePacket extends Packet {
    ...
    public void parseFromInput(DataInput in) throws IOException {
        ...
        for (int i = 0; i < this.drops.length; i++) this.drops[i] = in.readInt();
    }

    @Override
    public void writeToOutput(DataOutput out) throws IOException {
        out.writeShort(this.tiles.length);
        for (Tile tile : this.tiles) tile.writeToOutput(out);
        out.writeShort(this.newObjs.length);
        for (Entity obj : this.newObjs) obj.writeToOutput(out);
        out.writeShort(this.drops.length);
        for (int drop : this.drops) out.writeInt(drop);
    }
}