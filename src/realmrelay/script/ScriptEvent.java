...
public class ScriptEvent {
    ...
    public boolean connect(int gameId) {
        ...
        final InetSocketAddress socketAddress = RealmRelay.instance.getSocketAddress(gameId);
        new Thread(new Runnable() {
            @Override
            public void run() {
                Socket remoteSocket;
                if (RealmRelay.instance.bUseProxy) {
                    SocketAddress proxyAddress = new InetSocketAddress(RealmRelay.instance.proxyHost, RealmRelay.instance.proxyPort);
                ...
                try {
                    ...
                    if (InetAddress.getByName(socketAddress.getHostString()).isLoopbackAddress()) {
                        remoteAddress = new InetSocketAddress(RealmRelay.instance.remoteHost, socketAddress.getPort() == -1 ? RealmRelay.instance.remotePort : socketAddress.getPort());
                    } else {
                        remoteAddress = new InetSocketAddress(socketAddress.getHostString(), socketAddress.getPort() == -1 ? RealmRelay.instance.remotePort : socketAddress.getPort());
                    }
    ...
    public void echo(String message) {
        RealmRelay.echo(message);
    }

    public GroundData findGround(Object searchterm) {
        if (searchterm instanceof Number) {
            int type = (int) ((double) searchterm);
            return XmlParser.tileMap2.get(type);
        }
        return XmlParser.tileMap.get(searchterm.toString().toUpperCase());
    }

    public ItemData findItem(Object searchterm) {
        if (searchterm instanceof Number) {
            int type = (int) ((double) searchterm);
            return XmlParser.itemMap2.get(type);
        }
        return XmlParser.itemMap.get(searchterm.toString().toUpperCase());
    }

    public ObjectData findObject(Object searchterm) {
        if (searchterm instanceof Number) {
            int type = (int) ((double) searchterm);
            return XmlParser.objectMap2.get(type);
        }
        return XmlParser.objectMap.get(searchterm.toString().toUpperCase());
    }

    public byte findPacketId(String name) {
        Integer id = (Integer) XmlParser.packetMap.get(name.toUpperCase());
        if (id == null) return -1;
        return id.byteValue();
    }
    ...
    public void setGameIdSocketAddress(int gameId, String host, int port) {
        RealmRelay.instance.setSocketAddress(gameId, host, port);
    }
